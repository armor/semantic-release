...
module.exports = ({branch, options: {tagFormat}}, {before} = {}) => {
  const preRegExp = new RegExp(`.*-${branch.prerelease}\\.(\\d+)$`);
  const [{version, gitTag, gitHead, channel} = {}] = branch.tags
    .filter(tag => (branch.type === 'prerelease' && branch.channel === tag.channel && tag.version.match(preRegExp)) || !semver.prerelease(tag.version))
